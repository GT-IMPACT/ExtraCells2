...
public class WidgetFluidSelector extends AbstractFluidWidget {
	...
	private void drawHollowRectWithCorners(int posX, int posY, int height, int width, int color, int thickness) {
		drawRect(posX, posY, posX + height, posY + 2, color);
		drawRect(posX, posY + width - thickness, posX + height, posY + width, color);
		drawRect(posX, posY, posX + 2, posY + width, color);
	...
	public boolean drawTooltip(int posX, int posY, int mouseX, int mouseY) {
		if (this.fluid == null || this.amount <= 0 || !isPointInRegion(posX, posY, this.height, this.width, mouseX, mouseY)) {
			return false;
		}

		String amountToText = NumberFormat.getInstance().format(this.amount) + "L";

		List<String> description = new ArrayList<>();
		description.add(this.fluid.getLocalizedName(new FluidStack(this.fluid, 0)));
		description.add(amountToText);
		drawHoveringText(description, mouseX - this.guiFluidTerminal.guiLeft(), mouseY - this.guiFluidTerminal.guiTop() + 18, Minecraft.getMinecraft().fontRenderer);
		return true;
	}
